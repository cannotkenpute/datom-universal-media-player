  ...
  (let [Kd (or diffuse [1.0 1.0 1.0])
        ...
        (reduce
          (fn [[ar ag ab dr dg db sr sg sb] light]
            ...
            ;; accumulating.
            (let [kind (:kind light)
                  lc (when-not (= kind :hemisphere)
                       (srgb->linear (conj (vec (:color light)) 1.0)))
              ...
              (case kind
                ...
                :hemisphere (let [sky (srgb->linear (conj (vec (:sky-color light)) 1.0))
                                  ground (srgb->linear (conj (vec (:ground-color light)) 1.0))
                                  normal-y (double (nth normal 1))
                                  hemi-val (+ (* normal-y 0.5) 0.5)
                                  h-r (+ (* (nth ground 0) (- 1.0 hemi-val)) (* (nth sky 0) hemi-val))
                                  h-g (+ (* (nth ground 1) (- 1.0 hemi-val)) (* (nth sky 1) hemi-val))
                                  h-b (+ (* (nth ground 2) (- 1.0 hemi-val)) (* (nth sky 2) hemi-val))]
                              [(+ ar (* h-r intensity))
                               (+ ag (* h-g intensity))
                               (+ ab (* h-b intensity)) dr dg db sr sg sb])
...
(defn shade-mesh-fragment
  "Shade a single fragment for a mesh draw. Returns nil when alpha cutout
   discards the fragment; otherwise returns [r g b a] in sRGB,
  ...
  (let [op (:op draw)
    ...
    (when (>= (nth tex-result 3) (double (get op :alpha/cutoff 0.0)))
