  ...
  (let [kind (:op/kind op)
    ...
    (case kind
      ...
      (do
        ...
        (when-let [cutoff (:alpha/cutoff op)]
          (when-not (v/in-unit-interval? cutoff)
            (v/reject! "draw3d/mesh :alpha/cutoff must be in [0, 1]")))
         ...
         (case kind
           ...
           :light/ambient (do (when-not (v/valid-light-color? (:color op))
                              ...
                              (recur (next ops)
                                     ...
                                     lighting-enabled?))
           :light/hemisphere
           (do (when-not (v/valid-light-color? (:sky-color op))
                 (v/reject! "light/hemisphere :sky-color must be [r g b]"))
               (when-not (v/valid-light-color? (:ground-color op))
                 (v/reject! "light/hemisphere :ground-color must be [r g b]"))
               (recur (next ops)
                      model-stack
                      clip-depth
                      target-stack
         ...
         (let [op (first ops)
           ...
           (recur
             ...
             (case kind
               ...
               :light/hemisphere (update state
                                         :lights
                                         conj
                                         {:kind :hemisphere,
                                          :sky-color (:sky-color op),
                                          :ground-color (:ground-color op),
