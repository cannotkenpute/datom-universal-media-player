...
(defn layout
  ...
  (let [w (max 320.0 (double (or viewport-width 320)))
        ...
        controls-height (if (= mode :compact) 176.0 104.0)]
    {:layout/mode mode
     :viewport/width w
     :viewport/height h
     :controls/x 12.0
...
(defn- volume-form
  [state x y width]
  (let [volume (if (:player/muted? state)
                 0.0
                 (double (:player/volume state 1.0)))]
    (translated
      x y
      [:stack
       [:rect {:width width
               :height 12
               :color (:track colors)
               :node-id :player/volume
               :interactive-events #{:tap}}]
       (when (pos? volume)
         [:rect {:width (* width volume)
                 :height 12
                 :color (:accent colors)}])])))
...
(defn ui-form
  ...
  (let [layout* (layout state viewport)
        ...
        volume-y (+ button-y (if compact? 54.0 13.0))
    ...
    [:stack
     ...
     (label 24 (- viewport-height 30.0)
            (or source-name "datom.world media") 18 (:text colors))
     (label 24 (- viewport-height 56.0) (status-copy state) 13
            (if (= :error (:player/status state))
              (:danger colors)
              (:muted colors)))
     (button :player/open-file
             (- viewport-width 132.0)
             (- viewport-height 58.0)
             108.0
             "Open file")
     (translated
       x y
     ...
     (button :player/play (+ x 16.0) button-y 64.0 "Play")
     (button :player/pause (+ x 88.0) button-y 64.0 "Pause")
     (button :player/mute (+ x 160.0) button-y 72.0 mute-label)
     (volume-form state
                  (+ x (if compact? 16.0 240.0))
                  volume-y
                  (if compact? 128.0 96.0))
     (button :player/rate
             (+ x (if compact? 240.0 344.0))
             button-y
             64.0
             rate-label)
     (button :player/fullscreen (- (+ x width) 104.0)
             (+ button-y (if compact? 48.0 0.0))
             88.0
