...
(deftest accepts-hemisphere-light
  (is (vector? (lower/validate-frame! [{:op/kind :light/hemisphere,
                                        :sky-color [1.0 1.0 1.0],
                                        :ground-color [0.5 0.5 0.5]}]))))


(deftest rejects-invalid-hemisphere-light
  (is (thrown? #?(:clj Exception
                  :default :default)
        (lower/validate-frame! [{:op/kind :light/hemisphere,
                                 :sky-color [2.0 1.0 1.0],
                                 :ground-color [0.5 0.5 0.5]}]))))
...
(deftest alpha-cutoff-is-validated-and-lowered
  (let [frame [{:op/kind :camera3d/set,
               ...
               {:op/kind :draw3d/mesh,
                ...
                :indices [[0 1 2]],
                :uvs [[0 0] [1 0] [0 1]],
                :texture/source :sprite,
                :alpha/cutoff 0.5}]
        lowered (lower/lower-frame frame
                                   {:viewport-width 100
                                    :viewport-height 100
                                    :resolve-resource (fn [source _] source)})
        draw (second (get-in lowered [:passes 0 :draws]))]
    (is (= 0.5 (get-in draw [:op :alpha/cutoff])))
    (is (thrown? #?(:clj Exception :default :default)
                 (lower/validate-frame!
                   [(assoc (second frame) :alpha/cutoff 1.01)])))))
